    ...
    if (!isset($error)) {
        try {
            ...
            if ($res->num_rows > 0) {
                ...
                $sql = 'INSERT INTO users (name, fullname, email, phone, password) VALUE (?, ?, ?, ?, sha1(?))';
                $stmt = $conn->prepare($sql);
...
<div id="contents-with-footer">
    <section id="content-app" class="py-1">
        <div class="container registration__box">
            <div class="row">
                <div class="col-4 mx-auto bg-white rounded shadow-sm">
                    ...
                    ?>
                    <h2 class="mt-4 text-center">Registration Form</h2>
                    <form action="" method="POST" class="mt-4 mb-1">
                        ...
                        <input class="mt-4 w-100 rounded-pill btn-primary" type="submit" name="registration" value="Registration">
                    </form>
                    <a href="./login.php" class="mb-4 registration__link">You already have an account <i class="fa-solid fa-arrow-right-long"></i></a>
                </div>
            </div>
        </div>
    </section>
