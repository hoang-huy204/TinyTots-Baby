...
try {
    $sql = 'SELECT COUNT(id) as qty_product FROM products WHERE status = "active"';
    $res = $conn->query($sql);
    if ($res) {
        $row = $res->fetch_assoc();
        ...
        // get product
        $sql = 'SELECT products.id, products.name, products.price, products.description, products.image, categories.name as category FROM products INNER JOIN categories ON categories.id = products.category_id WHERE products.status = "active" ORDER BY products.id LIMIT ?, ?';
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ii', $page_first_result, $results_per_page);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res->num_rows > 0) {
            $lst_product_active = $res->fetch_all(MYSQLI_ASSOC);
        } else {
            die('Get data from table products failed');
        }
    }
} catch (Exception $e) {
    ...
    <div class="bg-white admin-controls">
        <div id="admin__nav" class="">
            ...
            <ul class="list-unstyled text-capitalize">
                ...
                <li class="mb-1 admin__nav-item">
                    <a href="./products.php" class="p-2 rounded admin__nav-item-link admin__nav-item-link--active">Products</a>
                </li>
                <li class="mb-1 admin__nav-item">
            ...
            <div class="col-md-12 mt-4">
                <div class="p-3 bg-white rounded">
                    <div class="d-flex align-items-center justify-content-between">
                        <h3>Products</h3>
                        <div>
                            <a href="./products_deleted.php" class="btn btn-danger">Deleted products</a>
                            <a href="./change_product.php" class="btn btn-success">Add product</a>
                        </div>
                    </div>
                    ...
                    <div class="table">
                        ...
                        <div class="table-body">
                            <?php
                            foreach ($lst_product_active as $product) :
                            ?>
                                <div class="table-row">
                                    ...
                                    <div class="table-cell">
                                        <div class="d-flex">
                                            <button class="d-inline-block btn btn-warning edit-product-btn" data-id="<?= $product['id'] ?>">Edit</button>
                                            <button class="d-inline-block ms-2 btn btn-danger del-product-btn" data-id="<?= $product['id'] ?>">Delete</button>
                                        </div>
                                    </div>
                                </div>
                            <?php
