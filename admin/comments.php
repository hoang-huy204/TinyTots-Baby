...
try {
    $sql = 'SELECT COUNT(id) as qty_comment FROM comments';
    $res = $conn->query($sql);
    if ($res) {
        $row = $res->fetch_assoc();
        $number_of_result = $row['qty_comment'];
        $results_per_page = 8;
        ...
        $stmt->execute();
        $res = $stmt->get_result();
        $comments_data = $res->fetch_all(MYSQLI_ASSOC);
    }
} catch (Exception $e) {
    ...
    <div class="bg-white admin-controls">
        <div id="admin__nav" class="">
            ...
            <ul class="list-unstyled text-capitalize">
                ...
                <li class="mb-1 admin__nav-item">
                    <a href="./comments.php" class="p-2 rounded admin__nav-item-link admin__nav-item-link--active">Comments</a>
                </li>
                <li class="mb-1 admin__nav-item">
                        ...
                        <?php
                            ...
                            ?>
                                <div class="table-row">
                                    ...
                                    <div class="table-cell"><?= $comment['fullname'] ?></div>
                                    <div class="table-cell"><?= $comment['email'] ?></div>
                                    <div class="table-cell"><?= $comment['message'] ?></div>
                                </div>
                            <?php
                            $count++;
                            endforeach;
